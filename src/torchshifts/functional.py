...
try:
    module_path = Path(__file__).parent / (f'shifts_cpu.{"pyd" if platform.system() == "Windows" else "so"}')
...
try:
    module_path = Path(__file__).parent / (f'shifts_cuda.{"pyd" if platform.system() == "Windows" else "so"}')
    torch.ops.load_library(str(module_path))
    ...
    shift3d_backward_cuda = torch.ops.shifts_cuda.shift3d_backward_cuda
...
def shift_dispatcher(dimension, *args):
    ...
    _type ='backward' if  len(tensor_args) > 2 and _device != 'quantize' else 'forward'
