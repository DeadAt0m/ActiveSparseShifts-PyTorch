...
template<typename T>
__device__ __forceinline__ T mod(T a, T b){return (b + (a % b)) % b;}

template<typename idx_t>
__device__ __forceinline__ idx_t infer_index(idx_t index, idx_t len, BIPadding padding_mode){
    if (len == 1){return 0;}
...
__device__ __forceinline__ scalar_t get_shifted_value(idx_t i_shifted, idx_t sizeH, idx_t strideH,
...
template<typename scalar_t, typename idx_t>
__device__ __forceinline__ scalar_t* get_shifted_values(idx_t i_shifted, idx_t sizeH, idx_t strideH,
                                                        idx_t j_shifted, idx_t sizeW, idx_t strideW,
                                                        idx_t k_shifted, idx_t sizeD, idx_t strideD,
                                                        idx_t c, idx_t strideC,
                                                        scalar_t* array, scalar_t zero_point, 
...
__device__ __forceinline__ scalar_t compute_interpolated(scalar_t* v, scalar_t diff_shiftH, scalar_t diff_shiftW, scalar_t diff_shiftD,
...
template <typename scalar_t, typename idx_t>
__device__ __forceinline__ scalar_t* compute_weight_gradient(scalar_t* v, scalar_t diff_shiftH, scalar_t diff_shiftW, scalar_t diff_shiftD,
                                                             idx_t sizeH, idx_t sizeW, idx_t sizeD, scalar_t zero_point){
...
__device__ __forceinline__ void shift_forward_kernel_nchwd(scalar_t* input, scalar_t* output,
...
__device__ __forceinline__ void shift_backward_kernel_nchwd(scalar_t* input_grad, scalar_t* input,  scalar_t* output_grad,
    ...
    scalar_t *weights_grad_tmp = compute_weight_gradient<scalar_t,idx_t>(tmp, dshifts[0], dshifts[1], dshifts[2],
                                                                         sizeH, sizeW, sizeD, zp);
    ADD((weights_grad + c*weights_grad_sC),(input_grad_NCHWD_val * weights_grad_tmp[0]));
    if (sizeW>1){ADD((weights_grad + c*weights_grad_sC + weights_grad_sS),(input_grad_NCHWD_val * weights_grad_tmp[1]));}
    if (sizeD>1){ADD((weights_grad + c*weights_grad_sC + 2*weights_grad_sS),(input_grad_NCHWD_val * weights_grad_tmp[2]));}
}


template <typename scalar_t, typename idx_t, bool quantized, bool active>
__device__ __forceinline__ void shift_forward_kernel_nhwdc(scalar_t* input, scalar_t* output, 
                                                            idx_t* weights, scalar_t* dweights,
...
template <typename scalar_t, typename idx_t, bool active>
__device__ __forceinline__ void shift_backward_kernel_nhwdc(scalar_t* input_grad, scalar_t* input,  scalar_t* output_grad,
                                                            idx_t* weights, scalar_t* dweights, scalar_t* weights_grad,
                                                            idx_t n, idx_t i, idx_t j, idx_t k,
                                                            idx_t sizeC, idx_t sizeH, idx_t sizeW, idx_t sizeD,
                                                            idx_t input_grad_sN, idx_t input_grad_sC, idx_t input_grad_sH, idx_t input_grad_sW, idx_t input_grad_sD,
    ...
    for (idx_t c = 0; c < sizeC; c++)
    {
        ...
        ADD((weights_grad + c*weights_grad_sC),(input_grad_NHWDC_val * weights_grad_tmp[0]));
        if (sizeW>1){ADD((weights_grad + weights_grad_sS + c*weights_grad_sC),(input_grad_NHWDC_val * weights_grad_tmp[1]));}
        if (sizeD>1){ADD((weights_grad + 2*weights_grad_sS + c*weights_grad_sC),(input_grad_NHWDC_val * weights_grad_tmp[2]));}
    }
}


template <typename in_t, typename out_t, bool q_type, bool active>
__device__ __forceinline__ out_t* init_weights(in_t* src, const int size)
{
...
__device__ __forceinline__ scalar_t* init_weight_offsets(scalar_t* src, const int size)
{
    scalar_t *dst = new scalar_t[size];
    for (int i = 0; i < size; ++i) 
    {
