...
API_INCLUDE void shift_forward_kernel_nchwd(scalar_t* input, scalar_t* output,
    ...
    } STATIC_ELSE
    {
        ...
        val = compute_interpolated<scalar_t,idx_t,false>(_vals_array, dshifts[0], dshifts[1], dshifts[2],
                                                         sizeH, sizeW, sizeD);
    } STATIC_ENDIF
    *output_NCHWD = val;
}
...
API_INCLUDE void shift_backward_kernel_nchwd(scalar_t* input_grad, scalar_t* input,  scalar_t* output_grad,
    ...
    STATIC_IF(active)
    {
         ...
         *output_grad_NCHWD = compute_interpolated<scalar_t,idx_t, true>(_vals_array, dshifts[0], dshifts[1], dshifts[2],
                                                                         sizeH, sizeW, sizeD);
    } STATIC_ELSE
    {                                                              
...
API_INCLUDE void shift_forward_kernel_nhwdc(scalar_t* input, scalar_t* output, 
    ...
    for (idx_t c = 0; c < sizeC; c++)
    {
        ...
        {   
            ...
            val = compute_interpolated<scalar_t,idx_t,false>(_vals_array, *(dweights+c*dweights_sC), *(dweights+dweights_sS+c*dweights_sC),
...
API_INCLUDE void shift_backward_kernel_nhwdc(scalar_t* input_grad, scalar_t* input,  scalar_t* output_grad,
    ...
    for (idx_t c = 0; c < sizeC; c++)
    {
        ...
        STATIC_IF(active)
        {
            ...
            *(output_grad_NHWD+c*output_grad_sC) = compute_interpolated<scalar_t,idx_t,true>(_vals_array, dshifts[0], dshifts[1], dshifts[2],
                                                                                              sizeH, sizeW, sizeD);
        } STATIC_ELSE
        {
